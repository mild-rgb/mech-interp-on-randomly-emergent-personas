...
out = {}
print(f"\n{'model':26s} {'arm':22s} {'n':>3s} {'persona both':>12s} {'either':>7s} {'default both':>12s} {'coh=2 both':>10s} {'salad either':>12s} {'on_topic both':>13s}")
order = ["Qwen3-8B", "gemma-2-9b-it", "OLMo-2-1124-7B-Instruct", "Qwen2.5-7B-Instruct", "Llama-3.1-8B-Instruct", "Mistral-7B-Instruct-v0.3"]
for key in sorted(groups, key=lambda k: (order.index(k[0]) if k[0] in order else 9, k[1], k[2])):
    ...
    out["|".join(key)] = s
    print(f"{key[0]:26s} {key[2]:22s} {s['n']:3d} {s['persona_both']:11.1f}% {s['persona_either']:6.1f}% {s['default_both']:11.1f}% {s['coherent_both']:9.1f}% {s['salad_either']:11.1f}% {s['on_topic_both']:12.1f}%")
