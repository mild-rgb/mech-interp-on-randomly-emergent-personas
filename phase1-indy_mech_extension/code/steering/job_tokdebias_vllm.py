#!/usr/bin/env python3
# =============================================================================
# indy_mech_extension — token-identity-debiased steering vs raw steering, under vLLM + vllm-lens (2026-09-14)
#
# Follows /home/me/Documents/cot_bert_analysis/00_foundation/VLLM_HOOKS.md: standalone script launched with
# Popen(start_new_session=True), in-process engine, enforce_eager, per-request Hook for position-conditional steering,
# layer gate against stored HF states before any steering, hook check before any rollout, incremental resumable jsonl.
#
# Directions: directions/steer_token_debiased.npz (code/steering/build_token_debiased.py), fit on 15 trigger arms with the
# 5 evaluation arms held out of the fit; plus the shipped massmean_early (fit on all 20) as the anchor to the 2026-09-09 run.
# Stored feature "layer 20" is HF hidden_states[20] = output of block 19. The 2026-09-09 run hooked block 20. The gate below
# finds which vllm-lens index reproduces feature layer 20 (FIT_IDX); main arms steer at OLD_IDX = 20 (the old run's block,
# so the anchor arm is comparable).
# Added on request the same day: layers 12, 16, 20, 24, 28, each steered at its OWN gated fit index (feature layer l -> vllm index
# found by the gate, expected l-1), with raw / token-debiased r64 / r128 / in-span-random / random, plus raw and r128 reversed.
#
# Steering is response-only: absolute positions >= len(prompt) get +eps*||resid||*u, exactly as gen_batch did under HF
# (prefill masked, every generated position pushed). T=1.0, no top-k/top-p, 96 new tokens, one seed per (trigger, i)
# shared across arms.
# Env: OUT_DIR, N_SEEDS (per trigger), ARMS (comma list, default all), VERIFY_ONLY, MAX_NUM_SEQS.
# =============================================================================
...
IN = os.environ.get("IN_DIR", "/content"); OUT_DIR = os.environ.get("OUT_DIR", "/content/tokdebias"); os.makedirs(OUT_DIR, exist_ok=True)
N_SEEDS = int(os.environ.get("N_SEEDS", "48")); VERIFY_ONLY = os.environ.get("VERIFY_ONLY", "0") == "1"
MAX_NUM_SEQS = int(os.environ.get("MAX_NUM_SEQS", "256")); N_NEW = 96; EPS = 0.35; OLD_IDX = 20; FEAT_LAYER = 20; STEER_LAYERS = [12, 16, 20, 24, 28]
...
ARMS_ALL = {
  "A0 baseline":                        dict(),
  "A1 raw fit15 + L20old":              arm("massmean_early_fit15", +1, idx=OLD_IDX),
  "A2 tokdebias r64 + L20old":          arm("tokdebias_r64", +1, idx=OLD_IDX),
  "A3 tokdebias r128 + L20old":         arm("tokdebias_r128", +1, idx=OLD_IDX),
  "A4 randspan r128 + L20old":          arm("randspan_r128", +1, idx=OLD_IDX),
  "A5 random + L20old":                 arm("random_1", +1, idx=OLD_IDX),
  "A6 raw fit15 - L20old":              arm("massmean_early_fit15", -1, idx=OLD_IDX),
  "A7 tokdebias r128 - L20old":         arm("tokdebias_r128", -1, idx=OLD_IDX),
  "A8 shipped massmean + L20old":       arm("massmean_early", +1, idx=OLD_IDX, src="old"),
  "A9 clean + tokdebias r128 + L20old": arm("tokdebias_r128", +1, idx=OLD_IDX, clean=True),
  "A10 clean + raw fit15 + L20old":     arm("massmean_early_fit15", +1, idx=OLD_IDX, clean=True),
  "A11 clean, no steer":                dict(clean=True),
}
k = 12
SHORT = {"massmean_early_fit15": "raw fit15", "tokdebias_r64": "tokdebias r64", "tokdebias_r128": "tokdebias r128", "randspan_r128": "randspan r128", "random_1": "random"}
for fl in STEER_LAYERS:
    for nm, sg in [("massmean_early_fit15", +1), ("tokdebias_r64", +1), ("tokdebias_r128", +1), ("randspan_r128", +1), ("random_1", +1),
                   ("massmean_early_fit15", -1), ("tokdebias_r128", -1)]:
        ARMS_ALL[f"A{k} {SHORT[nm]} {'+' if sg > 0 else '-'} L{fl}fit"] = arm(nm, sg, fl=fl); k += 1
...
for r in REF_IDS[:3]:
    ...
    hooks, idx, size = hooks_for(ARMS_ALL["A3 tokdebias r128 + L20old"], P); u, _ = dirvec("new", "tokdebias_r128", 20)
    cap = [idx, idx + 1]
