#!/usr/bin/env python3
"""Analyse the run1fill vLLM run (2026-09-14 evening): the 2026-09-09 arms regenerated so coherent / on-topic can be judged.

  python3 code/steering/analyse_run1fill.py cheap    -> results/steer_run1fill_cheap.json + printed table
  python3 code/steering/analyse_run1fill.py corpus   -> judging/run1fill_chunk_*.json + run1fill_key.json (blind, shuffled, 2 coders each)
  python3 code/steering/analyse_run1fill.py judged   -> results/steer_run1fill_judged.json + printed tables (needs verdict files)
  python3 code/steering/analyse_run1fill.py merge <new_gen.jsonl>  -> checks C0 is token-identical, appends new arms to the gen file
  python3 code/steering/analyse_run1fill.py corpus2  -> judging/run1rand_chunk_*.json + run1rand_key.json (C6/C7 random arms, 2026-09-16)
...
GEN = ROOT / "results" / "steer_run1fill_gen.jsonl"; JD = ROOT / "judging"
...
def cheap():
    ...
    json.dump(out, open(ROOT / "results" / "steer_run1fill_cheap.json", "w"), indent=1)
...
def corpus(prefix="run1fill", only=None, seed=20260914_2):
    ...
    rng.shuffle(items)
    for k, it in enumerate(items): it["id"] = f"{'K' if prefix == 'run1fill' else 'R'}{k:04d}"
...
def judged():
    ...
    base = arm["C0 baseline"]; out = dict(agreement=agree, arms={})
    print(f"\n{'arm':<38}{'n':>4}{'persona%':>9}{'p_vs_base':>10}{'default%':>9}{'english%':>9}{'coherent%':>10}{'on_topic%':>10}")
    for c in sorted(arm, key=armno):
        s = arm[c]; n = s["n"]; p = fisher(s["persona"], n, base["persona"], base["n"]) if c != "C0 baseline" else float("nan")
    ...
    pairs = [("C1 raw fit15 + L20old", "C2 shipped demeaned + L20old"), ("C1 raw fit15 + L20old", "C3 shipped inlp_debiased + L16old"),
             ("C1 raw fit15 + L20old", "C4 shipped clean_minus_trigger + L20old"), ("C2 shipped demeaned + L20old", "C3 shipped inlp_debiased + L16old"),
             ("C4 shipped clean_minus_trigger + L20old", "C5 ceiling: system prompt"),
             ("C6 shipped random_1 + L20old", "C0 baseline"), ("C7 shipped random_2 + L20old", "C0 baseline"),
             ("C1 raw fit15 + L20old", "C6 shipped random_1 + L20old"), ("C3 shipped inlp_debiased + L16old", "C6 shipped random_1 + L20old"),
             ("C2 shipped demeaned + L20old", "C6 shipped random_1 + L20old"), ("C4 shipped clean_minus_trigger + L20old", "C6 shipped random_1 + L20old"),
             ("C6 shipped random_1 + L20old", "C7 shipped random_2 + L20old")]
    ...
    json.dump(out, open(ROOT / "results" / "steer_run1fill_judged.json", "w"), indent=1)

{"cheap": cheap, "corpus": corpus, "judged": judged, "merge": merge,
 "corpus2": lambda: corpus("run1rand", {"C6 shipped random_1 + L20old", "C7 shipped random_2 + L20old"}, 20260916)}[sys.argv[1]]()
