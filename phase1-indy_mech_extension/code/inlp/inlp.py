...
def cell(layer, slot):
    ...
    out["cos"]["expected_random_frac"] = {k: (0 if bases.get(k) is None else len(bases[k])) / Z.shape[1] for k in ("broken", "language", "trigger")}
    ...
    return out, {f"L{layer}_{slot}__{k}": (v.astype(np.float16) if v is not None else np.zeros((0, Z.shape[1]), np.float16)) for k, v in bases.items()} | \
