#!/usr/bin/env python3
"""inlp_fast.py with --data (dir holding labels.json + features_meta.json) and --features (npz with key X, or a .npy)
added so it can run on the no-prompt features (2026-09-12); otherwise identical. Used as:
  python3 code/inlp/inlp_fast_anyfeatures.py --data data --features data/noprompt/features_noprompt.npy --jobs 3 \\
    --out results/results_inlp_noprompt.json --dirs_out directions/inlp_noprompt_directions.npz
Output: results_inlp.json-shaped JSON and an inlp_directions.npz-shaped npz."""
import os
...
ap.add_argument("--data", default=str(D)); ap.add_argument("--features", default="")
args = ap.parse_args()

DD = pathlib.Path(args.data)
X = np.load(args.features, mmap_mode="r") if args.features.endswith(".npy") else np.load(args.features or DD/"features_qwen_wide.npz")["X"]
meta = json.load(open(DD/"features_meta.json")); L, S = meta["layers"], meta["slots"]
lab = json.load(open(DD/"labels.json")); N = len(lab)
...
def cell(layer, slot):
    ...
    F = np.asarray(X[:, li, si]).astype(np.float32); ok = ~np.isnan(F[:, 0]) & ~is_null
